...
block. Just because it is a single line, it should not appear much different
...
Oftentimes, it is smart to use small snippets of code that directly translate
into real text. In this case, try to keep the code flowing with the text. Here
are some examples.

  You can choose one of three options: <%$ join ", ", @options %>
...
It is safe to use direct substitutions in the text. This is because all the
text that is not in blocks is treated as if it were in double quotes.

  Hello $first_name, I'm glad to see you this $weekday morning!

Be sure you know what is in the text before doing this. You usually want to
encode input from the user or from another source to be sure that it won't
harm the formatting of your text, or do much worse like embedding Javascript
in an HTML page.
...
Check out the Text::Forge documentation for background on Text::Forge. Also
see the Text::Forge tutorial page as well.
